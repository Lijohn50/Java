...
public class DressController implements Initializable {
    ...
    public void saveEvent()
    {
        ...
        else
        {
            ...
            savedText.setText("Dress Information Saved in TXT File...");

            String line = name + "," + type + "," + color + "," + price + "," + date + "," + quantity + "," + boost + "," + discount + "," + details + "," + gender + "," + size + "," + imagePath + '\n';
            try
            {
                RandomAccessFile raf = new RandomAccessFile("dressCollection.txt", "rw");
                raf.seek(raf.length());
                raf.writeBytes(line);
                raf.close();
            }
